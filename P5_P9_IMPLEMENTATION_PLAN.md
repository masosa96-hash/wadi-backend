...
export async function summarizeFile(fileText: string) {
  return await openai.chat.completions.create({
    ...
    messages: [{
      role: 'system',
      content: 'Resumí el siguiente documento en 3-5 puntos clave en español:'
    }, {
      role: 'user',
      content: fileText
    }]
...
export async function trackMessageUsage(userId: string, tokensUsed: number, model: string) {
  const metrics = await supabase.rpc('get_or_create_current_usage', { p_user_id: userId });
