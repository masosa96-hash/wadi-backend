          ...
          <div className={`flex items-center gap-2 mt-1.5 px-1 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
            ...
            {!isUser && (
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button className="p-1 hover:bg-white/5 rounded text-gray-600 hover:text-gray-400 transition-colors" title="Copy">
                  <Icons.Copy />
                </button>
  ...
  const handleSend = (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const aiResponse: Message = {
        ...
        role: 'assistant',
        content: 'Entendido. Procesando solicitud... \n\nHe analizado tus requerimientos y aquí tienes un plan preliminar basado en la estructura que mencionaste.',
        timestamp: new Date()
      };
  ...
  return (
    <div className="flex h-screen w-full bg-[#0D0D0F] text-[#E4E4E7] font-sans overflow-hidden selection:bg-cyan-500/20 selection:text-cyan-200">
      ...
      <main className="flex-1 flex flex-col relative z-10 h-full min-w-0">
        ...
        {/* Chat Area */}
        <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-24 scrollbar-thin scrollbar-thumb-[#1A1A1D] scrollbar-track-transparent">
