...
exports.getConversationFiles = getConversationFiles;
...
async function downloadFile(req, res) {
    try {
        ...
        // Get file metadata
        const { data: file, error: dbError } = await supabase_1.supabase
            .from("files")
            .select("*")
            .eq("id", fileId)
            .eq("user_id", userId)
        ...
        const { data: fileData, error: storageError } = await supabase_1.supabase.storage
...
async function deleteFile(req, res) {
    try {
        ...
        // Delete from storage
        await supabase_1.supabase.storage.from(file.storage_bucket).remove([file.storage_path]);
        // Delete from database (cascade will handle queue entries)
        const { error: deleteError } = await supabase_1.supabase.from("files").delete().eq("id", fileId);
        if (deleteError) {
