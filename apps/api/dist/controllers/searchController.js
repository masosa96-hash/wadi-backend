...
async function globalSearch(req, res) {
    try {
        ...
        const { error: refreshError } = await supabase_1.supabase.rpc("refresh_search_index");
        if (refreshError) {
        ...
        const { data: results, error: searchError } = await supabase_1.supabase.rpc("global_search", {
            p_user_id: userId,
            p_query: searchQuery,
...
async function getSearchSuggestions(req, res) {
    try {
        ...
        const { data: conversations, error: convError } = await supabase_1.supabase
            ...
            .limit(5);
        const { data: workspaces, error: workspaceError } = await supabase_1.supabase
            .from("workspaces")
            .select("name")
            .eq("owner_id", userId)
...
async function getMessageContext(req, res) {
    try {
        ...
        const { data: contextMessages, error: contextError } = await supabase_1.supabase.rpc("get_message_context", {
