...
exports.removeProjectTag = removeProjectTag;
exports.addRunTag = addRunTag;
exports.removeRunTag = removeRunTag;
 ...
 */
async function addProjectTag(req, res) {
    try {
        const userId = req.user_id;
        ...
        // Verify tag belongs to user
        const { error: tagError } = await supabase_1.supabase
            .from("tags")
            .select("id")
            .eq("id", tag_id)
            .eq("user_id", userId)
 ...
 */
async function removeProjectTag(req, res) {
    try {
        const userId = req.user_id;
        const projectId = req.params.id;
 ...
 * Adds a tag to a run
 */
async function addRunTag(req, res) {
    try {
        const userId = req.user_id;
        ...
        const { error: runError } = await supabase_1.supabase
            .from("runs")
            .select("id")
            .eq("id", runId)
            .eq("user_id", userId)
 ...
 */
async function removeRunTag(req, res) {
    try {
        const userId = req.user_id;
        const runId = req.params.id;
        const tagId = req.params.tagId;
