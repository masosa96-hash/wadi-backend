...
exports.executePreset = executePreset;
...
async function createPreset(req, res) {
    try {
        ...
        if (project_id) {
            const { data: projectData, error: projectError } = await supabase_1.supabase
                .from("projects")
                .select("id")
                .eq("id", project_id)
                .eq("user_id", userId)
...
async function updatePreset(req, res) {
    try {
        ...
        const { data: existingPreset, error: presetError } = await supabase_1.supabase
            .from("presets")
            .select("id")
            .eq("id", presetId)
            .eq("user_id", userId)
        ...
        if (model !== undefined)
            updates.model = model;
        if (folder !== undefined)
            updates.folder = folder?.trim() || null;
 ...
 */
async function executePreset(req, res) {
    try {
        const userId = req.user_id;
        const presetId = req.params.id;
        ...
        // Get preset
        const { data: preset, error: presetError } = await supabase_1.supabase
            .from("presets")
            .select("*")
            .eq("id", presetId)
            .or(`user_id.eq.${userId},is_public.eq.true`)
