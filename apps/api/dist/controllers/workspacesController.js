...
async function createWorkspace(req, res) {
    try {
        ...
        // Create workspace
        const { data: workspace, error: workspaceError } = await supabase_1.supabase
            .from("workspaces")
            .insert({
            owner_id: userId,
            name: name.trim(),
        ...
        if (memberError) {
            ...
            await supabase_1.supabase.from("workspaces").delete().eq("id", workspace.id);
...
async function getWorkspaceMembers(req, res) {
    try {
        ...
        // Verify user is a member
        const { data: memberCheck, error: memberCheckError } = await supabase_1.supabase
            .from("workspace_members")
            .select("id")
            .eq("workspace_id", workspaceId)
            .eq("user_id", userId)
...
async function removeMember(req, res) {
    try {
        ...
        const { data: targetMember, error: targetError } = await supabase_1.supabase
            .from("workspace_members")
            .select("role, user_id")
            .eq("id", memberId)
            .eq("workspace_id", workspaceId)
