...
exports.checkFileLimit = checkFileLimit;
...
async function checkMessageLimit(req, res, next) {
    try {
        ...
        const { data, error } = await supabase_1.supabase.rpc("check_usage_limit", {
            p_user_id: userId,
        ...
        console.log(`[LimitCheck] User ${userId} within limit: ${result.current_usage}/${result.limit_value}`);
        next();
    }
    catch (error) {
...
async function checkFileLimit(req, res, next) {
    try {
        ...
        console.log(`[LimitCheck] User ${userId} within file limit: ${result.current_usage}/${result.limit_value}`);
        next();
    }
    catch (error) {
        console.error("[LimitCheck] Exception:", error);
...
async function checkWorkspaceLimit(req, res, next) {
    try {
        ...
        console.log(`[LimitCheck] User ${userId} within workspace limit: ${result.current_usage}/${result.limit_value}`);
        next();
    }
    catch (error) {
        console.error("[LimitCheck] Exception:", error);
 ...
 */
async function checkFileSizeLimit(userId, fileSizeMB) {
    try {
